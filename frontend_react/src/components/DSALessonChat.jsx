...
const IconConstruction = ({ size = 28, color = "#f59e0b" }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
export default function DSALessonChat({
  ...
  const [internalViewTab, setInternalViewTab] = useState('theory')
  const viewTab = activeSubtab || internalViewTab
  const setViewTab = (tab) => {
    setInternalViewTab(tab)
    if (onSubtabChange) onSubtabChange(tab)
  }

  const isAdmin = !guestUser && userEmail && userEmail.toLowerCase() === 'agniva.sardar@gmail.com'
    ...
    <div className="dsa-lesson-chat-container">
      ...
      <div className="dsa-lesson-header">
        ...
        <div className="dsa-view-subtabs">
          ...
          >
            <IconTerminal /> {isAdmin ? 'Interactive Lab & Practice IDE' : <><IconLock /> Interactive Lab IDE (Under Construction)</>}
          </button>
        </div>
      ...
      <div className="dsa-lesson-stream">
        {viewTab === 'theory' ? (
          ...
          isAdmin ? (
            ...
            <div className="dsa-locked-container" style={{ padding: '3.5rem 1.5rem', textAlign: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.08)', margin: '1.5rem 0' }}>
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
                <IconConstruction size={32} color="#f59e0b" />
                <IconLock size={32} color="#38bdf8" />
              </div>
              ...
              <p style={{ color: '#aaa', fontSize: '0.9rem', maxWidth: '520px', margin: '0 auto 1.5rem', lineHeight: '1.6' }}>
                This feature is currently under construction and available exclusively for authorized developer accounts.
              </p>
              <button className="dsa-btn-action solid" onClick={() => setViewTab('problems')} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                <IconBack /> Back to Problems & Solutions
              </button>
            </div>
          )
        ) : (
