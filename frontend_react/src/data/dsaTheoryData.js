export const dsaTheoryData = {
  ...
  4: {
    ...
    basics: [
      ...
      },
      {
        ...
        detail: "Insert at Head in O(1) time.",
        code: "// Insert at Head\nNode* newNode = new Node(val);\nnewNode->next = head;\nhead = newNode;\n// Time: O(1)"
      },
      {
        op: "3. Deletion Operations",
        detail: "Delete node after target pointer in O(1) time.",
        code: "// Delete node after 'prev'\nNode* temp = prev->next;\nprev->next = temp->next;\ndelete temp;\n// Time: O(1)"
      }
    ],
    patterns: [
      {
        ...
        codeWalkthrough: "• Line 3: Move slow 1 step.\n• Line 4: Move fast 2 steps.\n• Line 5: If pointers meet, cycle exists.",
        approach: "1. `slow = head`, `fast = head`.\n2. Move slow by 1, fast by 2.",
        timeComplexity: "O(N)",
        spaceComplexity: "O(1)",
        whenToApply: "Cycle detection, finding middle node.",
        whenNotToApply: "Arrays with direct index access."
      },
      {
        name: "2. Reverse Linked List",
        explanation: "Reverses pointer links iteratively in O(N) time and O(1) space.",
        code: "Node* prev = nullptr;\nNode* curr = head;\nwhile (curr) {\n    Node* nextTemp = curr->next;\n    curr->next = prev; // Reverse pointer\n    prev = curr;\n    curr = nextTemp;\n}\nreturn prev; // New head",
        codeWalkthrough: "• Line 4: Store next node reference.\n• Line 5: Reverse current node's next pointer.\n• Line 6-7: Advance prev and curr pointers.",
        approach: "1. Maintain `prev`, `curr`, `nextTemp`.\n2. Re-link `curr->next = prev`.",
        timeComplexity: "O(N)",
        spaceComplexity: "O(1)",
        whenToApply: "Reversing linked lists, palindrome linked list check.",
        whenNotToApply: "Trees with parent/child non-linear structures."
      }
    ],
    complexities: [
      { operation: "Head Insertion", time: "O(1)", space: "O(1)" }
    ],
    strategy: "Dummy heads eliminate null pointer checks. Fast & slow pointers solve cycle and middle node problems."
  },
  ...
  7: {
    ...
    basics: [
      {
        op: "1. Stack & Queue Operations",
        detail: "Stack: LIFO (`push`, `pop`, `top`). Queue: FIFO (`push`, `pop`, `front`).",
    ...
    patterns: [
      {
        ...
        explanation: "Maintains stack elements in strictly increasing/decreasing order to find Next Greater / Smaller Element in O(N) time.",
        code: "vector<int> nextGreater(n, -1);\nstack<int> st;\nfor (int i = 0; i < n; i++) {\n    while (!st.empty() && arr[i] > arr[st.top()]) {\n        nextGreater[st.top()] = arr[i];\n        st.pop();\n    }\n    st.push(i);\n}",
        codeWalkthrough: "• Line 4: While current element is greater than element at stack top, pop stack top and set its next greater element to arr[i].\n• Line 8: Push current index to stack.",
        approach: "1. Maintain stack of indices.\n2. Pop elements smaller than current element.",
        timeComplexity: "O(N) total",
        spaceComplexity: "O(N)",
        whenToApply: "Next Greater Element, Stock Span, Histogram Area.",
  ...
  11: {
    ...
    basics: [
      {
        op: "1. Node Definition & Traversals",
        detail: "Preorder (Root-Left-Right), Inorder (Left-Root-Right), Postorder (Left-Right-Root).",
    ...
    patterns: [
      {
        ...
        code: "int maxDiameter = 0;\nint getHeight(TreeNode* root) {\n    if (!root) return 0;\n    int lh = getHeight(root->left);\n    int rh = getHeight(root->right);\n    maxDiameter = max(maxDiameter, lh + rh);\n    return 1 + max(lh, rh);\n}",
        codeWalkthrough: "• Line 4-5: Recursively compute left and right subtree heights.\n• Line 6: Update diameter at current node as lh + rh.",
  ...
  13: {
    ...
    basics: [
      {
        op: "1. Adjacency List Representation",
        detail: "`vector<int> adj[V]` stores neighbors of each vertex.",
        code: "vector<vector<int>> adj(V);\nadj[u].push_back(v);\nadj[v].push_back(u); // For undirected graph"
      }
    ],
    patterns: [
      {
        ...
        code: "vector<int> vis(V, 0);\nqueue<int> q;\nq.push(startNode);\nvis[startNode] = 1;\nwhile (!q.empty()) {\n    int node = q.front(); q.pop();\n    for (int neighbor : adj[node]) {\n        if (!vis[neighbor]) {\n            vis[neighbor] = 1;\n            q.push(neighbor);\n        }\n    }\n}",
        codeWalkthrough: "• Line 3-4: Enqueue start node and mark visited.\n• Line 7-11: Process neighbors level by level.",
        approach: "1. Enqueue root and mark visited.\n2. Process neighbors via Queue.",
        timeComplexity: "O(V + E)",
        spaceComplexity: "O(V)",
        whenToApply: "Unweighted shortest path, level-order graph traversal.",
        whenNotToApply: "Weighted graphs with varying edge costs (use Dijkstra)."
      }
    ],
    complexities: [
      { operation: "BFS / DFS", time: "O(V + E)", space: "O(V)" }
    ],
    strategy: "Use BFS for unweighted shortest path, DFS for connectivity and cycle detection."
  },
  ...
  14: {
    ...
    basics: [
      {
        op: "1. DP Principles",
        detail: "1. Overlapping Subproblems 2. Optimal Substructure",
        code: "// 1D DP Fibonacci Tabulation\nvector<int> dp(n + 1);\ndp[0] = 0; dp[1] = 1;\nfor (int i = 2; i <= n; i++) {\n    dp[i] = dp[i - 1] + dp[i - 2];\n}"
      }
    ],
    patterns: [
      {
        ...
        code: "vector<vector<int>> dp(n + 1, vector<int>(W + 1, 0));\nfor (int i = 1; i <= n; i++) {\n    for (int w = 0; w <= W; w++) {\n        if (wt[i-1] <= w) {\n            dp[i][w] = max(dp[i-1][w], val[i-1] + dp[i-1][w - wt[i-1]]);\n        } else {\n            dp[i][w] = dp[i-1][w];\n        }\n    }\n}",
        codeWalkthrough: "• Line 5: Max of excluding item dp[i-1][w] or including item val[i-1] + dp[i-1][w-wt[i-1]].",
        approach: "1. State `dp[i][w]`.\n2. Recurrence: `max(exclude, include)`.",
        timeComplexity: "O(N * W)",
        spaceComplexity: "O(N * W) or O(W)",
        whenToApply: "Subset sum, target sum, partition equal subset sum.",
        whenNotToApply: "Fractional items (use Greedy)."
      }
    ],
    complexities: [
  ...
  15: {
    ...
    basics: [
      {
        op: "1. Trie Node Definition",
        detail: "`struct TrieNode { TrieNode* children[26]; bool isWord; }`.",
    ...
    patterns: [
      {
        ...
        explanation: "Traverses character children pointers in O(L) time where L is word length.",
        code: "void insert(string word) {\n    TrieNode* curr = root;\n    for (char c : word) {\n        int idx = c - 'a';\n        if (!curr->children[idx]) curr->children[idx] = new TrieNode();\n        curr = curr->children[idx];\n    }\n    curr->isWord = true;\n}",
        codeWalkthrough: "• Line 4-5: Follow or create character child pointer at index c - 'a'.\n• Line 7: Mark end of word.",
        approach: "1. Traverse character pointers.\n2. Mark `isWord = true` at word end.",
        timeComplexity: "O(L)",
    ...
    complexities: [
      { operation: "Trie Search / Insert", time: "O(L)", space: "O(N * L * 26)" }
    ],
  ...
  16: {
    ...
    basics: [
      {
        op: "1. Pattern Search Mechanics",
        detail: "Linear pattern matching algorithms eliminate quadratic backtracking."
      }
    ],
    patterns: [
      {
        ...
        codeWalkthrough: "• Line 5-7: Precomputes longest proper prefix that is also suffix for pattern p.",
        approach: "1. Build `lps` array in O(M).\n2. Search text in O(N).",
